...
extension TodosAPI {
    ...
    // 특정 할 일 가져오기
    static func fetchATodoWithObservable(id: Int, completion: @escaping
    ...
    // 모든 할일 검색하기
    static func searchTodosWithObservable(searchTerm: String, page: Int = 1, completion: @escaping
    ...
    // - isDone: 할일 완료형
    // - completion: 응답 결과
    static func addATodoWithObservable(title: String, isDone: Bool = false, completion: @escaping
                         (Result<BaseResponse<Todo>, ApiError>) -> Void) {
        // 1. urlRequest를 만든다
    ...
    // - isDone: 할일 완료형
    // - completion: 응답 결과
    static func addATodoJsonWithObservable(title: String, isDone: Bool = false, completion: @escaping
                             (Result<BaseResponse<Todo>, ApiError>) -> Void) {
        // 1. urlRequest를 만든다
    ...
    // - isDone: 완료여부
    // - completion: 응답 결과
    static func editTodoJsonWithObservable(id: Int, title: String, isDone: Bool = false, completion: @escaping
                             (Result<BaseResponse<Todo>, ApiError>) -> Void) {
        // 1. urlRequest를 만든다
    ...
    // - isDone: 완료여부
    // - completion: 응답 결과
    static func editTodoWithObservable(id: Int, title: String, isDone: Bool = false, completion: @escaping
                         (Result<BaseResponse<Todo>, ApiError>) -> Void) {
        // 1. urlRequest를 만든다
        let urlString = baseURL + "/todos/\(id)"
        guard let url = URL(string: urlString) else {
    ...
    // - id:  삭제할 아이템 아이디
    // - completion: 응답 결과
    static func deleteATodoWithObservable(id: Int, completion: @escaping
                            (Result<BaseResponse<Todo>, ApiError>) -> Void) {
    ...
    ///   - isDone:
    ///   - completion:
    static func addTodoAndFetchTodosWithObservable(title: String,
                                     isDone: Bool = false,
